app.controller("order-ctrl", function($scope, $http, $location) {
	...
	$scope.edit = function(item) {
		...
		$http.get(`/rest/orders/tickets/${item.id}`).then(resp => {
	...
	$scope.updateOrder = function(){
		if($scope.order.id == "" || $scope.order.id == null){
			Swal.fire({
				icon: 'error',
				title: 'Thất bại',
			...
			Swal.fire({
			  title: 'Bạn có muốn xuất hóa đơn này?',
			  showCancelButton: true,
			  confirmButtonText: 'Xác nhận',
			}).then((result) => {
			  if (result.isConfirmed) {
				$scope.order.time = new Date("1974-03-12T"+$scope.order.time);
				console.log($scope.order);
				$http.put("/rest/orders/update", $scope.order).then(resp => {
