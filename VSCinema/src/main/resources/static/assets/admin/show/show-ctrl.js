app.controller('show-ctrl', function($scope, $http) {
	...
	$scope.fills = function() {
		...
		if (roomid != '' && movieid != '' && listshowdate != '') {
			if (listshowdate.length >= 10) {
				listshowdate = listshowdate.slice(6, 10) + "-" + listshowdate.slice(3, 5) + "-" + listshowdate.slice(0, 2);
			}
			$scope.fill = [];
			for (var i = 0; i < $scope.items.length; i++) {
				console.log($scope.items[i].movie.id + "==" + movieid && $scope.items[i].room.name + "==" + roomid && $scope.items[i].showDate + "==" + listshowdate)
		...
		if (movieid != '' && listshowdate != '') {
			...
			if (listshowdate.length == 4) {
				...
				for (var i = 0; i < $scope.items.length; i++) {
					...
					console.log($scope.items[i].movie.id + "==" + movieid + "\n&&" + new Date($scope.items[i].showDate).getFullYear() + "==" + listshowdate);
				}
			}
			if (listshowdate.length != 4) {
