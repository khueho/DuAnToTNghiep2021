...
app.controller("booking-ticket-ctrl" , function($scope , $http){
	...
	$scope.getFilm = function(item){
		$scope.listShow = [];
		$http.post("/rest/show/showtimes", item).then(resp =>{
          	$scope.listShow = resp.data;
			$scope.listFilm = [];
			for(var i=0; i<$scope.listShow.length;i++){
				var film = $scope.listFilm.find(f => f.id == $scope.listShow[i].movie.id);
				if(film == null){
					$scope.listFilm.push($scope.listShow[i].movie);
				}
			}
		});
	}
	
	$scope.getFilmByToday = function(){
		$scope.listShow = [];
		$http.get("/rest/show/today").then(resp =>{
          	$scope.listShow = resp.data;
			$scope.listFilm = [];
			for(var i=0; i<$scope.listShow.length;i++){
				var film = $scope.listFilm.find(f => f.id == $scope.listShow[i].movie.id);
