app.controller('sale-ctrl', function ($scope, $http) {
	...
	$scope.editSale = function (item) {
		$scope.reset();
		$scope.dataUpdate = angular.copy(item);
	...
	$scope.update = function () {
		...
		if(item.id == null){
			Swal.fire({
				icon: 'error',
				title: 'Thất bại',
				text: 'Vui lòng chương trình khuyến mãi.',
				width: '500px',
				heightAuto: true
			})
		} else if($scope.dataUpdate.endDate < today){
			Swal.fire({
				icon: 'error',
				title: 'Thất bại',
				text: 'Chương trình đã hết hạn! Không thể cập nhật.',
				width: '500px',
				heightAuto: true
			})
	...
	$scope.create = function () {
		...
		if(item.food.id == null){
			...
			Swal.fire({
				icon: 'error',
				title: 'Thất bại',
				text: 'Chương trình khuyến mãi đã tồn tại!',
				width: '500px',
				heightAuto: true
			})
			...
			for(var i=0;i < $scope.items.length;i++){
				if($scope.items[i].food.id == item.food.id){
					$scope.items[i].endDate = new Date($scope.items[i].endDate);
					var today = new Date();
					if($scope.items[i].endDate > today){
						$scope.checkSale.push($scope.items[i]);
					}
				}
			}
			if($scope.checkSale.length > 0){
				Swal.fire({
					icon: 'error',
					title: 'Thất bại',
					text: 'Sản phẩm đã trong chương trình khác!',
					width: '500px',
					heightAuto: true
				})
			} else {
