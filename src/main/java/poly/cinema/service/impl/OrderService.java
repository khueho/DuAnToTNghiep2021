...
public class OrderService implements poly.cinema.service.OrderService {
	...
	public List<Ticket> findTicketByOrderId(String orderid) {
		return ticketDAO.findByOrderId(orderid);
	}
	...
	public Order update(Order order) {
		Order o = dao.findById(order.getId()).get();
		o.setActive(4);
		if(o.getTickets().size() > 0) {
			o.getTickets().forEach(t -> {
				t.setActivity(false);
			});
		}
		dao.save(o);
        ...
        if(vnp_ResponseCode.equals("00")) {
        	if(vnp_TransactionStatus.equals("00")) {
        		...
        		if(order.getTickets().size() == 0) {
                    ...
                    try {
            			mailerService.send(order.getUser().getEmail(), "THÔNG TIN HÓA ĐƠN", "<!DOCTYPE html>\n"
            					+ "<html>\n"
            					+ "<head>\n"
	...
	@Override
	public UrlRequest createOrderFood(CreateOrderFoodRequest data) throws UnsupportedEncodingException {
		Calendar cld = Calendar.getInstance(TimeZone.getTimeZone("Etc/GMT+7"));
		List<Order> listOrder = dao.findAll();
		List<Ticket> list = ticketDAO.findAll();
