...
public class ShowServicempl implements ShowService {
	...
	@Override
	public List<Show> getFilm(ShowtimesRequest showtimes) {
		return showdao.getShowByDate(showtimes.getDate());
	}

	@Override
	public List<Show> getShowByToday() {
		Calendar cld = Calendar.getInstance(TimeZone.getTimeZone("Etc/GMT+7"));
		Date now = cld.getTime();
		return showdao.getShowByToday(now);
	}
