...
public interface ShowDAO extends JpaRepository<Show, Integer> {
	...
	@Query("Select s from Show s where s.room.name = :roomname")
	List<Show> findByRoom(String roomname);

	@Query("Select s from Show s where s.movie.id = :movieid and s.room.name = :roomname")
	List<Show> findByMovieAndRoom(Integer movieid, String roomname);

	@Query("Select s.showDate from Show s group by s.showDate")
	...
	@Query("Select s from Show s where s.showDate = :showdate and s.movie.id=:id")
	...
	List<Show> getShowByDate(Date date);

	@Query("select s from Show s where s.showDate=:now")
	List<Show> getShowByToday(Date now);
